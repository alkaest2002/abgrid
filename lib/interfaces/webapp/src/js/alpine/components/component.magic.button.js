    ...
    magicButton: {
        ...
        [":class"]() {
            const isDisabled = this.$store.abgrid.magicButtonIsLocked
                ...
                : "cursor-pointer";
            const isDisabledMeta = this.$store.abgrid.apiRequestIsPending
                ? "opacity-25 cursor-not-allowed"
                : "cursor-pointer";
        ...
        async ["@click"]({ metaKey }) {
            ...
            if (metaKey && !this.$store.abgrid.apiRequestIsPending) {
                // Unlock magic button
            ...
            if (this.$store.abgrid.magicButtonIsLocked || this.$store.abgrid.apiRequestIsPending)
                return;
            // Lock magic button

...
export default (label, action) => ({
    ...
    magicButtonClasses: "h-18 w-18 rounded-full text-sm font-medium text-white border-0 shadow-xl inline-flex justify-center items-center backdrop-blur-sm transform hover:scale-105 transition-all duration-300 ease-out",
    ...
    magicButton: {
        ...
        [":class"]() {
            // Define color themes for different button states
            const primaryTheme = "bg-blue-600/90 hover:bg-blue-700/90 focus:ring-blue-300/50";
            const alternateTheme = "bg-purple-600/90 hover:bg-purple-700/90 focus:ring-purple-300/50";
            
            // Define disabled/enabled states
            const disabledStyles = "opacity-25 cursor-not-allowed";
            const enabledStyles = "cursor-pointer";
            
            // Determine interaction state based on app/API status
            const isPrimaryDisabled = this.$store.app.appMagicButtonIsLocked || this.$store.api.apiRequestIsPending;
            const isAlternateDisabled = this.$store.api.apiRequestIsPending;
            
            const primaryInteractionState = isPrimaryDisabled ? disabledStyles : enabledStyles;
            const alternateInteractionState = isAlternateDisabled ? disabledStyles : enabledStyles;
            
            // Compute final CSS classes based on meta key state
            const computedClasses = this.magicButtonMetaKeyIsPressed
                ? this.$store.api.apiRequestIsPending
                    ? `${alternateInteractionState} ${primaryTheme} ${this.magicButtonClasses}`
                    : `${alternateTheme} ${this.magicButtonClasses}`
                : `${primaryInteractionState} ${primaryTheme} ${this.magicButtonClasses}`;
        ...
        ["x-html"]() {
            const actualLabel = label instanceof Function
                ? label.call(this)
                : label;
            return this.magicButtonIsSpinning || this.$store.api.apiRequestIsPending
        ...
        async ["@click.stop"]({ metaKey }) {
            ...
            if (this.$store.app.appMagicButtonIsLocked || this.$store.api.apiRequestIsPending)
                return;
            // Lock magic button

...
export default () => ({
        ...
        try {
            ...
            if (method !== "GET" && bodyData != null) {
                ...
                const wantCompression =
                    compress === true ||
                    (compress === "auto" && typeof jsonBody === "string" && jsonBody.length >= compressionThreshold);

                if (wantCompression) {
                    const compressedBlob = await this.apiCompressJson(jsonBody, compressionAlgorithm);
                    if (compressedBlob) {
                        fetchOptions.body = compressedBlob;
                        fetchOptions.headers["Content-Encoding"] = compressionAlgorithm;
    ...
    async apiCompressJson(jsonString, algorithm = "gzip") {
        try {

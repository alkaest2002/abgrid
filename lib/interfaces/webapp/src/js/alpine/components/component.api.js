export default () => ({
    ...
    async apiHandleRequest(request) {
        try {
            ...
            // Make api request
            await this.apiFetch(request);
        } catch (error) {
            // Save error api response to store
            this.$store.api.apiResponse = this.createResponse(500, "internal_error");
        }
    },
    ...
    async apiFetch(config) {
        ...
        if (typeof endpoint !== "string" || endpoint.includes("://")) {
            // Save error api response to store
            this.$store.api.apiResponse = this.createResponse(400, "invalid_endpoint");
        }
        ...
        if (!import.meta.env.DEV && !url.startsWith("https://")) {
            // Save error api response to store
            this.$store.api.apiResponse = this.createResponse(400, "insecure_url_in_production");
        }
        ...
        try {
            ...
            if (method !== "GET" && bodyData != null) {
                if (
                    ...
                    // Save error api response to store
                    this.$store.api.apiResponse = this.createResponse(400, "only_json_body_data_accepted");
                }
                try {
                    ...
                    // Save error api response to store
                    this.$store.api.apiResponse = this.createResponse(400, "invalid_json_body_data");
                }
            }

            // Switch on pending flag
            ...
            try {
                ...
                this.$store.api.apiResponse = this.createResponse(response.status, message, jsonResponse);
            
            } catch {
                // Save error api response to store
                this.$store.api.apiResponse = this.createResponse(500, "malformed_json_from_server");
            }
            ...
            const msg = err.name === "AbortError" 
                ...
                : "network_error";
            // Save error api response to store
            this.$store.api.apiResponse = this.createResponse(500, msg);
        
        } finally {
            // Switch off pending flag

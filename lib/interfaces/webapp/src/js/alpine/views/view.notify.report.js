        ...
        return this.$store.app.appIsElectron
            ...
            ? {
                    async ["@click.prevent"]() {
                        ...
                        spinner == "notifyReportPDFSpinner"
                            ? this.notifyReportPDFSpinner = true
                            : this.notifyReportJSONSpinner = true;
                        const response = await window.electronAPI.generateReport(data, typeOfData);
                        response && this.toastifyReportGeneration(response);
                        spinner == "notifyReportPDFSpinner"
                            ? this.notifyReportPDFSpinner = false
                            : this.notifyReportJSONSpinner = false;

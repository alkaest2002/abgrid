...
export default () => ({
    ...
    async init() {
        ...
        this.$store.app.appIsOnline && window.Taxi.preload(`/pages/${this.$store.app.appLanguage}.multistep.sociogram.html`);
        this.$store.app.appIsOnline && window.Taxi.preload(`/pages/${this.$store.app.appLanguage}.multistep.json.html`);
        this.$store.app.appIsOnline && window.Taxi.preload(`/pages/${this.$store.app.appLanguage}.multistep.report.html`);
    },

    serverStatusMessage: {
        ["x-html"]() {
            const electronVersion = this.$store.app.appElectronVersion;
            return this.$store.api.apiServerIsReachable
                ? this.$store.app.appIsElectron
                    ? `<span class='italic'>${electronVersion} ${this.$t('server.up')}</span>`
                    : `<span class='italic'>${this.$t('server.up')}</span>`
                : this.$store.app.appIsElectron
                    ? `<span class='italic'>${electronVersion} ${this.$t('server.down')}</span>`

...
export default () => ({
    ...
    init() {
        this.$store.app.magicButtonIsLocked = 
            Object.keys(this.$store.data.reportDataJSON).length == 0;
    },
        ...
        try {
            ...
            if (file.type === "application/x-yaml" && file.name.endsWith(".yaml")) {
                try {
                    ...
                    this.$store.data.reportDataFilename = file.name;
                    // Save JSON report data to store
                    this.$store.data.reportDataJSON = jsonReportData;
                    // Save YAML report data to store
                    this.$store.data.reportDataYAML = limitedContent;
                    // Unlock magic button
    ...
    dropzone: {
        ...
        ["@textarea:input"]({ detail }) {
            this.$store.data.reportDataYAML = detail;
        }
    },
    ...
    dropzoneDownloadLink: {
        ...
        ["@click.prevent"]() {
            ...
            this.$nextTick(() => {
                ...
                link.href = `data:application/x-yaml;base64,${btoa(this.$store.data.reportDataYAML)}`;
                link.download = this.$store.data.reportDataJSON?.group
                    ? `group_file_g${this.$store.data.reportDataJSON.group}.yaml`
                    : 'group_file_g1.yaml';
                link.click();
            });
        }
    ...
    dropzoneResetLink: {
        ["@click.prevent"]() {
            ...
            this.$store.data.reportDataJSON = {};
            // Lock magic button
        ...
        async ["action"]() {
            try {
                ...
                this.$store.api.apiRequest = {
                    ...
                    bodyData: JSON.parse(JSON.stringify(this.$store.data.reportDataJSON))
                };

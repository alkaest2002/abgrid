...
export default () => ({
    ...
    dropzoneDownloadLink: {
        [":href"]() {
            if (!this.$store.abgrid.reportDataYAML) return '#';
            return `data:application/x-yaml;base64,${btoa(this.$store.abgrid.reportDataYAML)}`;
        },

        [":download"]() {
            if (!this.$store.abgrid.reportDataJSON?.group) return 'group_file_g1.yaml';
            return `group_file_g${this.$store.abgrid.reportDataJSON.group}.yaml`;
        },

        ["@click"]() {
            this.updateReportDataYAML();
        }
    },
    ...
    magicButton: {
        ...
        ["action"]() {
            try {
                ...
                console.log(this.$store.abgrid.reportDataJSON);
            } catch (error) {

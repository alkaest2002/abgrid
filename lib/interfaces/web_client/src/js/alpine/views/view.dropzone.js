...
export default () => ({
    ...
    readFileWithLineLimit(file, maxLines) {
            ...
            reader.onload = (evt) => {
                ...
                for (const line of lines) {
                    ...
                    if (lineCount >= maxLines) {
                        return reject(new Error(`File exceeds ${maxLines} lines limit`));
                    }
                }
        ...
        try {
            ...
            if (file.type === "application/x-yaml" && file.name.endsWith(".yaml")) {
                try {
                    ...
                    const limitedContent = await this.readFileWithLineLimit(file, 300);
                    ...
                    if (error.message.includes('exceeds') && error.message.includes('lines limit')) {
                        this.toastPush(400, this.$t("errors.file_too_large"));
                    } else {
